...
MqttClient.prototype.publish = async function(topic, payload, qos, retain) {
  const msgID = await Mqtt.publish(this.clientRef, topic, payload, qos, retain);
  return msgID;
};
MqttClient.prototype.publishUInt8 = async function(
  topic,
  payload,
  qos,
  retain
) {
  const msgID = await Mqtt.publishUInt8(
    this.clientRef,
    topic,
    payload,
    qos,
